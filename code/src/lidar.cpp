...
namespace r2d2::measuring_distance {
    ...
    void lidar_c::receive_measurement_data() {
        ...
        for (unsigned int count = 0; count < measurements_to_receive; count++) {
            // measurement_angle = starting_angle + (((measurement_count - 1) * 22.5) / total_points_per_frame)
            // This formula is extracted from the datasheet of the lidar communication protocol.
            measurements[((header.starting_angle / 10) + ((count * (225)) / ((header.data_length - 5) / 3)))/5] = {receive_uint8(), receive_uint16()};
        }
    }
