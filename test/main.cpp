...
TEST_CASE("module_c - send smallest distance"){
    std::vector<uint8_t> message = {
        0xAA, 0x00, 0x9A, 0x01, 0x61, 0xAD, 0x00, 0x92, 0x82, 0x00, 0x87, 0x69,
        0x78, 0x00, 0x00, 0x00, 0x46, 0x21, 0x3A, 0x54, 0x23, 0x78, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x91, 0x33, 0x60, 0x82, 0x32, 0xF7, 0x93, 0x32,
    ...
    for(int i=0; i<16; i++){
        // The module needs 16 measurement packets for one 360 degree measurment
        prepare_message(usart, message);
    }
    ...
    r2d2::distance::module_c module(mock_bus, lidar);

    mock_bus.request(r2d2::frame_type::DISTANCE);
    ...
    if(frames.size()> 0){
        ...
        const auto frame_data = frame.as_frame_type<r2d2::frame_type::DISTANCE>();
        REQUIRE(frame_data.mm == 2126);
    }
}
